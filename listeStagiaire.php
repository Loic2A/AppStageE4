...
class PageListeStagiaire extends PageListe
{
    ...
    protected function init()
    {
        ...
        if($this->userInfo['is_professeur'])
        {
    		$tableau->addColonne(new Colonne("Opération", 'etudiant.num_etudiant', '
    			<span class="center bouton_operation">
    				<a title="modifier" class="icon_modifier" href="modifierSpeEtu.php?id=$1"></a>
		...
		$tableau->addColonne(new Colonne("Spécialité", 'specialite.libelle', '$1'));
		
		$requete = " WHERE annee_obtention_bts IS NULL AND etudiant.en_activite = 1";
        ...
        if (isset($_POST['specialite']) && $_POST['specialite'] != "")
			$requete .= " AND etudiant.num_spec = ".$_POST['specialite'];
        ...
        $tableau_sql = new TableauSQL($tableau, $this->bdd, 
            "stage
				...
				LEFT OUTER JOIN specialite ON etudiant.num_spec = specialite.num_spec
                JOIN classe ON etudiant.num_classe = classe.num_classe"
                .$requete.
                " GROUP BY etudiant.num_etudiant ", 
